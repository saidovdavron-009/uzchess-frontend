...
const CATEGORIES = ["Barchasi", "Bullet", "Blitz", "Rapid", "Klassik"];
...
function FilterDropdown({label, value, options, onSelect}: {
    ...
    return (
        <div className="flex flex-col gap-4">
            <span className="font-poppins font-medium text-[12px] tracking-[0.72px] text-[#F7F9FA] uppercase">{label}</span>
...
export default function RankingFilters() {
    ...
    return (
        <div className="w-[326px] flex flex-col gap-6">
            <div className="w-[326px] h-[100px] flex items-center gap-3 justify-center rounded-[8px] bg-[#1A1D1F] border border-[#232627]">
                <Image src="/reyting-icon.svg" alt="" width={44} height={44} className="mt-[10px]"/>
                <h1 className="font-poppins font-bold text-[32px] leading-[41.6px] text-[#F7F9FA]">Reyting</h1>
            </div>
            ...
            <div className="w-[326px] h-auto px-5 pt-5 pb-5 rounded-[8px] bg-[#1A1D1F] border border-[#1F272A]">
                ...
                <div className="mt-6 flex flex-col gap-6">
                    <FilterDropdown label="Mamlakatni tanlang:" value={country} options={COUNTRIES} onSelect={setCountry}/>
                    <FilterDropdown label="Toifa:" value={category} options={CATEGORIES} onSelect={setCategory}/>
                </div>
            </div>
        </div>
    );
